...
int main(int argc, char *argv[])
{
    ...
    bool ignore = strcmp(argv[1], IGNORE_OUTPUT_SHORT) == 0 or
                  strcmp(argv[1], IGNORE_OUTPUT) == 0 or (argc >= 3 and (strcmp(argv[2], IGNORE_OUTPUT_SHORT) == 0 or strcmp(argv[2], IGNORE_OUTPUT) == 0));

    bool non_interactive = strcmp(argv[1], NON_INTERACTIVE_SHORT) == 0 or
                           strcmp(argv[1], NON_INTERACTIVE) == 0 or (argc >= 3 and (strcmp(argv[2], NON_INTERACTIVE_SHORT) == 0 or strcmp(argv[2], NON_INTERACTIVE) == 0));
    ...
    if (strcmp(argv[1], "-in") == 0 or strcmp(argv[1], "-ni") == 0)
    {
        ...
        argv = &argv[1 + ignore + non_interactive];
    ...
    else if (pid > 0)
    {
        ...
        ull elapsed = chrono::duration_cast<std::chrono::microseconds>(end - start).count();
        ...
        if (non_interactive)
            print_result_non_interactive(cerr, elapsed, user, sys, mem_max);
        else
            print_result_interactive(cerr, elapsed, user, sys, mem_max);
    }
    else
        error("fork");
